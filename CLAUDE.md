...
> - **Règle de répartition (50/30/20)** : éditable dans Réglages — **curseur** + pourcentage **et** montant côte à côte, tous liés (le montant = revenu cible × %, et inversement). Elle **pilote désormais les objectifs (« Prévu »)** : `ruleTargets(settings)` (`src/lib/store.tsx`) calcule Besoins/Envies/Épargne = revenu cible × %, utilisés par le Bilan (Prévu Fixes/Variables/Épargne), les barres du Budget et le bloc « Ce mois-ci » de l'Accueil. Tout découle donc d'un seul endroit : **revenu cible + pourcentages**.
...
> - **Historique mois par mois + prévu par catégorie** (décidé par Rin, 2026-06-21) :
>   - **Budget** et **Bilan** sont **filtrés par le mois sélectionné** (`MonthSelector` contrôlé → `value`/`onChange`). Dépenses & revenus sont datés (`date` ISO), filtrés par préfixe `YYYY-MM`. **Accueil** = mois courant. **Agenda** = mois sélectionnable (`monthInfoFor(prefix)`).
>   - **Charges fixes** = modèle récurrent (`charges`), mais **paiement historisé mois par mois** dans la table **`charge_payments` (charge_id, month, paid, amount)** ; `amount` = montant réel du mois (null = montant du modèle). Accès via `chargeState(chargeId, month, fallback)` / `setChargePaid` / `setChargeMonthAmount` (`store.tsx`, upsert sur clé unique `charge_id,month`). ⚠️ La colonne `charges.paid` (globale) n'est **plus utilisée** par l'UI.
>   - **Prévu par catégorie (enveloppes)** : colonne **`categories.budget`** (prévu mensuel), **coexiste** avec la règle 50/30/20. Édition dans Réglages (formulaire catégorie) **et** en ligne dans Budget (bloc « Prévu par catégorie »).
>   - **Migration BDD** : `docs/migration-002-historique-budgets.sql` (non destructif) ajoute `categories.budget` + `charge_payments` + RLS ; `docs/schema.sql` mis à jour pour les nouvelles installations.
...
> **Données** : magasin **en mémoire** `src/lib/store.tsx` (`DataProvider` + `useData()`), initialisé depuis `src/lib/mock.ts`. Collections gérées en CRUD complet : charges fixes/récurrences, dépenses variables, revenus (nature fixe/freelance), comptes d'épargne + versements, **catégories de dépenses personnalisables**, revenu cible, rappel. Édition des montants via `EditableAmount` (clic → champ), dates via sélecteur calendrier (`<input type="date">`, ISO + `formatDateShort`). Les modifs se reflètent partout **pendant la session**. ⚠️ **Non persistant** : réinitialisé au rechargement. Branchement Supabase prévu **à la toute fin**, une fois l'UX validée. Seul **Bilan** reste en lecture sur `mock.ts` (rapport) ; l'**Agenda** est dérivé du magasin (charges par jour-du-mois + dépenses par date ISO). La feuille « Saisie rapide » est pilotable via `src/lib/quickEntry.tsx` (`useQuickEntry().openSheet(dateISO?)`). Sur l'Agenda : **clic simple** sur un jour = voir ses opérations ; **appui long** (≥450 ms, pointer events) = ouvrir la saisie pré-remplie avec cette date. Le FAB ouvre la saisie sur la date du jour.
